...
export default function Dimensions ( props ) {
	...
	return (
        <div className="search-console-dimensions">
            <a className={ 'query' === selected ? 'is-selected' : '' } onClick={ () => onSelect( 'query' ) }>
                { __( 'Query', 'search-console' ) }
            </a>
            <a className={ 'page' === selected ? 'is-selected' : '' } onClick={ () => onSelect( 'page' ) }>
                { __( 'Pages', 'search-console' ) }
            </a>
            <a className={ 'country' === selected ? 'is-selected' : '' } onClick={ () => onSelect( 'country' ) }>
                { __( 'Countries', 'search-console' ) }
            </a>
            <a className={ 'device' === selected ? 'is-selected' : '' } onClick={ () => onSelect( 'device' ) }>
