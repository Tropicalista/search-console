...
namespace Tropicalista\SearchConsole\League\OAuth2\Client\Tool;

use Tropicalista\SearchConsole\GuzzleHttp\Exception\BadResponseException;
use Tropicalista\SearchConsole\GuzzleHttp\Psr7\Uri;
...
trait ProviderRedirectTrait
{
     ...
     */
    protected function followRequestRedirects(\Tropicalista\SearchConsole\Psr\Http\Message\RequestInterface $request)
    {
        $response = null;
        $attempts = 0;
        while ($attempts < $this->redirectLimit) {
            ...
            if ($this->isRedirect($response)) {
                $redirectUrl = new \Tropicalista\SearchConsole\GuzzleHttp\Psr7\Uri($response->getHeader('Location')[0]);
     ...
     * @return boolean
     */
    protected function isRedirect(\Tropicalista\SearchConsole\Psr\Http\Message\ResponseInterface $response)
    {
