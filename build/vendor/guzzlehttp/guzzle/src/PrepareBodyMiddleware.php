    ...
    public function __invoke(\Tropicalista\SearchConsole\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \Tropicalista\SearchConsole\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\Tropicalista\SearchConsole\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    private function addExpectHeader(\Tropicalista\SearchConsole\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
