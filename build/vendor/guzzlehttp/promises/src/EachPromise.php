 ...
 */
class EachPromise implements \Tropicalista\SearchConsole\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \Tropicalista\SearchConsole\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \Tropicalista\SearchConsole\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
